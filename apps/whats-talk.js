...
const PUSH_CRON = "0 12 * * *";
// 推送间隔时间（毫秒）
const PUSH_INTERVAL = 2 * 60 * 1000; // 2分钟
// 挖掘的历史消息
const HISTORY_LENS = 200;
// 推送的群组
const groupList = [374685489,778163948,434720657,1031609263,893539357,1025919424,608182376,961206440,1140119610,960096994,734293753,128334322,2150040360,820649351,982302589,913574559,722606642,807430880,511313238,836783295,807289446,961551604];
// ai 地址
const aiBaseURL = "https://api.9e.nz";
// ai Key
const aiApiKey = "sk-QfPjcjAbA05PK2DAzkeE5OwPodzJblSHI1wRdSNLCN4cCow3";
// ai 模型
    ...
    async pushWhatsTalk() {
        ...
        for (let i = 0; i < groupList.length; i++) {
            try {
                ...
                // 如果不是最后一个群组，等待2分钟再推送下一个
                if (i < groupList.length - 1) {
                    logger.info(`等待 ${PUSH_INTERVAL/1000/60} 分钟后推送下一个群组...`);
